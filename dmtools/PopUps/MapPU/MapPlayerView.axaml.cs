...
{
    public static event EventHandler upd0;
    ...
    public MapPlayerView(IMap thismap)
    {
        InitializeComponent();
        ...
        ini(thismap);
        MapInfo = thismap;
        MapEditor.IveBeenDeletedAAAA += MapEditorOnIveBeenDeletedAAAA;
        MapEditor.upd += MapEditorOnupd;
    }

    private void MapEditorOnupd(object? sender, EventArgs e)
    {
        ini(MapInfo);
    }

    private void MapEditorOnIveBeenDeletedAAAA(object? sender, EventArgs e)
    {
        this.Close();
    }
